...
class VendorController extends Controller
{
    ...
    public function developer_registration(Request $request){
        ...
        $request->validate([
            ...
            $developer->valid_days= $request->valid_days;
    ...
    { 
            ...
            if($developer){
                ...
                if ($result){
                    return  back()->with('status', 'Developer updated successfully!');                        
                    }else{
                    return  back()->with('status', 'Something Went Wrong!');
                    }
            }else{
    ...
    public function manage_budget(){
        $developers  =  Developer::all();
        $totalbudget = Developer_budget::sum('budget');
        $used_budget = Developer_budget::sum('used_amount');
        $remain_budget = Developer_budget::sum('remain_amount');
        $budgetList  = Developer_budget::select('developer_budgets.*','developers.name')->leftJoin('developers', 'developers.id', '=', 'developer_budgets.developer_id')->get();   
        

        return view('admin.manage-budget',compact('budgetList','developers','totalbudget','used_budget','remain_budget'));
    }
