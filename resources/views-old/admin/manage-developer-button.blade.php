        ...
        <div class="timeshare-section">
                        ...
                        <table class="table rejected-stimates-item">
                            ...
                            <tbody id = "row">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="ts-table-action"><a class="view-btn btn-danger" href="{{ url('admin/users-details') }}/{{ $ept['id'] }}">Delete</a></div>
                                    </td>
                                </tr>
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="timeshare-header">
                        <div class="mr-auto">
                            <h4 class="heading-title">Manage Developers Weekly Points </h4>
                        </div>
                        <div class="ts-filter wd30">
                            <div class="row g-2">
                                <div class="col-md-12">
                                    <div class="form-group">
                                        <a class="Create-btn" data-bs-toggle="modal" data-bs-target="#CreateWeeklyPoints">Create  Weekly Points</a>
                                    </div>
                                </div>
                            </div>
                        </div>
                                ...
                                <table class="table rejected-stimates-item">
                                    <thead>
                                        <tr>
                                            <th>Enroll Type </th>
                                            <th>Developer Name</th> 
                                            <th>Week Points Type</th>                                          
                                            <th>Status</th>                                            
                                            <th>Action</th>
                                        </tr>
                                    </thead>
                                    <tbody id = "row">
                                        @foreach($week_types as $ept)
                                        <tr>
                                            <td><div class="ts-table-text">{{$ept['point_type_title']}}</div></td>
                                            <td>
                                                <div class="ts-table-text">{{$ept['developers_name']}}</div>    
                                            </td>
                                            <td>
                                                <div class="ts-table-text">{{$ept['enrolled_points_title']}}</div>    
                                            </td>
                ...
                <div class="col-md-12">
                    <div class="timeshare-header">
                        <div class="mr-auto">
                            <h4 class="heading-title">Manage Developers OwenerShip Level </h4>
                        </div>
                        <div class="ts-filter wd30">
                            <div class="row g-2">
                                <div class="col-md-12">
                                    <div class="form-group">
                                        <a class="Create-btn" data-bs-toggle="modal" data-bs-target="#CreateOwenership">Add Owenership  Level</a>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="timeshare-card-body">
                            <div class="orders-table ts-table">
                                <table class="table rejected-stimates-item">
                                    ...
                                    <tbody id = "row">
                                        @foreach($owenership_level as $ept)
                                        <!-- {{print_r($enrolled_point_type)}} -->
                                        <tr>
                                            <td><div class="ts-table-text">{{$ept['ownership_type']}}</div></td>
                                            <td><div class="ts-table-text">{{$ept['developer_name']}}</div></td>
                                            @if($ept->weekly_type_id == '')
                                            <td><div class="ts-table-text">N/A</div></td>
                                            @endif
                                            @foreach($week_types as $wt)
                                            @if($ept->weekly_type_id == $wt->id)
                                            <td><div class="ts-table-text">{{$wt['point_type_title']}}</div></td>
                                            @endif
                                            @endforeach
                                            @if($ept->enroll_type_id == '')
                                            <td><div class="ts-table-text">N/A</div></td>
                                            @endif
                                            @foreach($enrolled_point_type as $wt)
                                            @if($ept->enroll_type_id == $wt->id)
                                            <td><div class="ts-table-text">{{$wt['point_type_title']}} </div></td>
                                            @endif
                                            @endforeach
                                            @if($ept->min_points == '')
                                            <td><div class="ts-table-text">N/A</div></td>
                                            @else
                                            <td><div class="ts-table-text">{{$ept['min_points']}}</div></td>
                                            @endif
                                            @if($ept->max_points == '')
                                            <td><div class="ts-table-text">N/A</div></td>
                                            @else
                                            <td><div class="ts-table-text">{{$ept['max_points']}}</div></td>  
                                            @endif                            
                                            <td>
                                                @if($ept['status'] == 1)
                                                    <div class="orders-price-text accepted-offer">Active</div>
...
<!-- start Create New Weekly Type-->
<div class="modal lm-modal fade" id="CreateWeeklyPoints" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-body">
        <div class="timeshare-modal-form">
            <h2>Add Owenership Week Type</h2>
            <form action="{{route('admin.weeklyType-registration')}}" class="developers_form" id="weeklyType-registration" method="post" enctype="multipart/form-data">
                @csrf 
            <div class="row">
                <div class="col-md-12">
                    <div class="form-group">
                        <select class="form-control" aria-label="Default select example" name="developer_id" id="developerid" required>
                ...
                <div class="col-md-12 enrollpointstype" style="display:none">
                    <div class="form-group">
                        <select class="form-control enrolledpointid" aria-label="Default select example" id="enrolled_point_type_id" name="enrolled_point_type_id" required>
                            <!-- <option value="" selected>Select Enroll Points Type</option>
                            @foreach($enrolled_point_type as $ept)
                            <option value="{{$ept->id}}">{{$ept->point_type_title}}</option>
                            @endforeach -->
                        </select>
                    </div>
                </div>
                <div class="col-md-8">
                    <div class="form-group">
                        <input type="text" class="form-control" name="point_type_title" placeholder="Weekly Points Type" required>
                    </div>
                </div>               
...
<!-- end Create New Weekly Type Level -->

<!-- start Create New Owenership Level -->
<div class="modal lm-modal fade" id="CreateOwenership" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-body">
        <div class="timeshare-modal-form">
            <h2>Add Owenership Type</h2>
            <form action="{{route('admin.ownership-registration')}}" class="developers_form" id="ownership-registration" method="post" enctype="multipart/form-data">
                @csrf 
            <div class="row">
                <div class="col-md-12">
                    <div class="form-group">
                        <select class="form-control" aria-label="Default select example" id ="developer_id" name="developer_id">
                            <option value="" selected>Select Developer</option>
                            @foreach($developers as $dev)
                ...
                <!-- <div class="col-md-12" class="" style="display:block">
                    <div class="form-group">
                        <select class="form-control" aria-label="Default select example" name="enroll_type_id" >
                            <option value="" selected>Select Enroll Point Type</option>
                            @foreach($enrolled_point_type as $eptt)
                            <option value="{{$eptt->id}}">{{$eptt->point_type_title}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>   -->
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        <input type="text" class="form-control @error('min_points') is-invalid @enderror" placeholder="Minimum Points" name="min_points">
                            <!--span class="{{$errors->first('name')?'error invalid-feedback':''}}">{{$errors->first('name')}}</span-->
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="form-group">
                        <input type="text" class="form-control" name="max_points" placeholder="Maximum Points">
                    </div>
                </div>
                <div class="col-md-8">
                    <div class="form-group">
                        <input type="text" class="form-control" name="ownership_type" placeholder="Owenership Level Name" required>
                    </div>
                </div>               
...
<!-- end Create New Owenership Level -->
